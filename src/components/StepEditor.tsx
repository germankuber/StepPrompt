...
const PromptSection = ({ 
  ...
  <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
    ...
    <div className="space-y-3">
      <div>
        <label className="block text-sm font-medium text-gray-600 mb-1">Prompt Content</label>
        <textarea
      ...
      <div className="flex items-center gap-2">
        <label className="flex items-center gap-2 cursor-pointer select-none">
          ...
          />
          <span className={clsx("text-sm", config.injectUserMessage ? "text-blue-700 font-medium" : "text-gray-500")}>
            Inject User Message
          </span>
        </label>
        {config.injectUserMessage && <User className="w-4 h-4 text-blue-500" />}
      </div>
    </div>
  </div>
);
...
export const StepEditor: React.FC<StepEditorProps> = ({ step, onUpdate, onDelete, isExpanded: initialExpanded, onToggleExpand }) => {
  ...
  return (
    <div className="bg-white shadow-md rounded-xl overflow-hidden border border-gray-100 transition-all">
        ...
        <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                <input 
                    ...
                    className="text-xl font-bold text-gray-800 bg-transparent border-none focus:ring-0 hover:bg-gray-100 rounded px-2"
                />
            </div>
        </div>
